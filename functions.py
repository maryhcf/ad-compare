...
def tucker_func(npall,caseno,ndim,latent_dim = 3,real_len=37):
    ...
    tic=time.perf_counter()
    for i in range(real_len+1,npall.shape[0]):
        core, factors = tucker(tl.tensor(npall[i,...]), rank=[latent_dim, latent_dim])
        recons[i,...]=tucker_to_tensor([core, factors])
        pred_mse[i]=np.square(recons[i,...]-npall[i,...]).sum()
    print('pred time')
...
def pca_func(all_flat,npall,real_len,caseno,ndim,latent_dim = 3):
    ...
    print(time.perf_counter()-tic)
    tic=time.perf_counter()
    pred_z=pca.transform(all_flat)[:,:latent_dim]
    pred_mse=np.sum((recons-npall)**2,axis=(1,2))
    print('pred time')
    print(time.perf_counter()-tic)
    vars = [pred_mse,recons,npall,pred_z]
    f=open(caseno+'.pkl','wb')
    pickle.dump(vars, f)
    f.close()
    return vars 
...
def glcm(caseno,npall,latent_dim,real_len):
    ...
    print(time.perf_counter()-tic)
    tic=time.perf_counter()
    for i in range(real_len,npall255.shape[0]):
        gm=greycomatrix(npall255[i,...], dis, ang, levels=npall255.max()+1)
        all_features[i,]=np.vstack([
            #greycoprops(gm,'energy'),
            #greycoprops(gm,'dissimilarity'),
...
def gan_func(caseno,filelist,ndim,train_ind,npinp_test,rand_shape=4,EPOCHS = 80,BUFFER_SIZE = 2000,BATCH_SIZE=256,num_out=3,test_len=10):
    ...
    train_size=int(BUFFER_SIZE*0.9)
    ...
    npinp_val=np.random.uniform(0,1,[real_len,ndim,ndim,1])
    k=0
    for i in train_ind:
        fi=filelist[i]
        npinpi = imageio.imread(fi)
        ...
        npinp_val[k,...]=npinpi
        npinp_val[k,...]=npinp_val[k,...]/255
        k=k+1
    npall=np.vstack([npinp_val,npinp_test])
    val2_dataset=tf.data.Dataset.from_tensor_slices((tf.convert_to_tensor(npall,dtype=tf.float32)))
    val2_dataset = val2_dataset.batch(1)
    val_img_size=npall.shape[0]
    deci=np.random.uniform(0,0,val_img_size)
    i=0
    print('fit time')
    print(time.perf_counter()-tic)
    tic=time.perf_counter()
    for val_2 in val2_dataset:
        deci[i]=discriminator(val_2).numpy()
        i=i+1
    print('pred time')
    print(time.perf_counter()-tic)
    ...
    plt.scatter(range(real_len+test_len,val_img_size),deci[(real_len+test_len):],color="g",marker="*",label="Testing images with simulated Anomaly")
...
def cvae_func(caseno,filelist,ndim,train_ind,npinp_test,latent_dim = 16,epochs = 200,BUFFER_SIZE = 1000,batch_size=128):
  ...
  val_img_size=len(train_ind)
  npinp_val=np.random.uniform(0,1,[val_img_size,ndim,ndim,1])
  k=0
  for i in train_ind:
      fi=filelist[i]
      npinpi = imageio.imread(fi)
  ...
  val_img_size=npall.shape[0]
  pred_mse_val=[]
  pred_z_val=np.random.uniform(0,0,[latent_dim,val_img_size])
  pred_mean_val=np.random.uniform(0,0,[latent_dim,val_img_size])
  pred_img_list=np.random.uniform(0,0,[ndim,ndim,val_img_size])
  i=0
  print('fit time')
  print(time.perf_counter()-tic)
  tic=time.perf_counter()
  for val_2 in val2_dataset:
      pred_mse_val.append(prediction_mse(model,val_2).numpy())
      pred_z_val[:,i]=latent_z(model,val_2).numpy()
      pred_mean_val[:,i]=latent_mean(model,val_2).numpy()
      pred_img_list[...,i]=(pred_x(model,val_2).numpy())[0,...,0]
      i=i+1
  print('pred time')
  print(time.perf_counter()-tic)
  tic=time.perf_counter()
  vars = [pred_z_val.transpose(),pred_img_list,pred_mse_val,pred_mean_val.transpose()] 
  f=open(caseno+'.pkl','wb')
  pickle.dump(vars, f)
  f.close()
  return vars 
